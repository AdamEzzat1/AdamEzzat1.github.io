...
description: "Part I of a two-article benchmark deep-dive on ABNG. This article covers the five benchmark categories where ABNG's properties can be proven correct at the tested scale: determinism (replay invariance under graph mutation, 2.40x Kahan summation overhead), lineage (cryptographic audit chain detecting single-row dataset tampering), state stability (NIG posterior converges 1/sqrt(n) to known truth), probabilistic consistency (graph-level ECE = 0.015 on a synthetic Bernoulli), and explainability (10x error reduction by abstaining on high-OOD predictions). The companion Part II covers adaptive graph evolution, routing behavior, and temporal state dynamics."
---
...
Five failure modes, ranked by severity and recoverability. Each is honest about what the article *doesn't* prove.

### 10.1 Single-hardware measurement (meta-limitation)

Every number in this article is from one machine (Intel i7-11390H, Windows MSVC release). The architecture's determinism canaries are locked on Windows MSVC and *should* hold cross-platform, but **cross-platform CI is unverified.** Phase 0.6 added a `cross-platform-determinism.yml` workflow skeleton; verification results have not been published. Until they are, every "Proven" tag in this article carries an implicit "on this hardware" footnote.

### 10.2 Audit-log size at production scale (extrapolation, not measurement)

Witness-only events are ~80 bytes each. Four state updates per observation (BLR, density, calibration, signature) at 1M rows produces ~320 MB of audit log. Fits on disk; painful in memory. Phase 0.7's `abng_compact_log` + `StatsSnapshot` markers + smart-replay address this, but the *unbounded-scale* behaviour (10⁹-event chains) is **unmeasured**. The linear-scaling claim is plausible but not validated past 10⁵.

### 10.3 No external lineage baseline

MLflow / DVC / Weights & Biases track *artefacts* (model files, dataset checksums). ABNG tracks *state mutations* (every parameter update has a hash witness). The granularity advantage is real but **never directly compared** — no head-to-head measurement of "regulator's nightmare scenario" against MLflow + a malicious lab. The Part 2 of the trilogy ([Deterministic and Auditable Neural Systems](../abng-deterministic-systems/index.qmd)) addresses this positionally; this article cannot address it empirically.

### 10.4 No predictive coverage analysis

The reliability diagram (§7.3) measures *calibration*. It does not measure *predictive coverage* — does the Student-t predictive distribution at a chosen confidence level (say, 95%) actually contain the true `y` at the rate it claims? This is a different test and requires drawing samples from the Student-t or computing exact CDF values. **Not implemented in this run.** Listed in §11 future work.

### 10.5 No statistical CIs on most per-op timings

Most per-operation costs in §9 are single-run measurements. The speedup comparisons (Phase 0.7 perf items, `observe_batch`, `route_to_leaf_batch`) use min-of-5 trials, which is more robust. Experiment α uses 7-trial measurements. The right framework is Criterion — automated median + IQR per bench. **Conversion to Criterion is a 4-hour task** listed in future work but not done in this run.

### Future work (Part I categories)

| Item | Cost | Closes which gap |
|---|---|---|
| Cross-platform CI verification (Linux + macOS + ARM) | 1–2 days | §10.1 |
| 10⁹-event audit log stress test | 1 day | §10.2 |
| sklearn-GP head-to-head on tabular | 1 day | (general external baseline) |
| Predictive coverage analysis | 3–4 hours | §10.4 |
| Convert microbench to Criterion (median + IQR per op) | 4 hours | §10.5 |
| MLflow / DVC tamper-scenario comparison | 1 day | §10.3 |
