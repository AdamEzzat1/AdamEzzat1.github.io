...
description: "Part 3 of a three-article series on ABNG. This piece is the empirical backbone — the demo catalog, the verification scorecard, six experiments measured on real hardware, and an honest accounting of every gap. New in this revision: posterior consistency on known truth (PINN posterior converges as 1/sqrt(n) toward analytical w_true), selective-prediction curve (abstaining on high-OOD predictions reduces mean error 10x), Kahan-vs-plain determinism premium quantified at exactly 2.40x, decide_step scaling sub-millisecond at all tested sizes, basis-in-span vs basis-not-in-span PINN comparison, and replay-under-mutation byte-identical verification. The whole architecture is still exploratory engineering, not validated science."
---
...
## TL;DR
A research-stage ML architecture is only as credible as its evidence. This article walks through the empirical state of ABNG: nine distinct demo categories totalling ~5,866 lines of test code, **six new experiments measured on real hardware** (NIG posterior consistency, basis-in-span vs out-of-span PINN, selective prediction on OOD, replay-under-mutation invariant, decide_step scaling, Kahan-vs-plain determinism premium), and the headline analytical-solution result (1D heat equation, MAE < 0.05 *on the architecture's home turf*). It is also brutally honest about what the demos *don't* prove: no external baselines (no sklearn, no PyTorch, no FNO comparisons), no large-N benchmarks beyond 100K rows, no cross-platform measurement, no statistical confidence intervals. The honest framing: **ABNG is exploratory engineering, not validated science**, and the closer names three specific experiments whose outcomes would change that.
:::
...
1. **Reliability diagrams and predictive coverage** (Items requested but not built — needs ~4 hours of additional instrumentation).
2. **External baselines.** sklearn-GP comparison, FNO comparison, Isolation Forest comparison, ADWIN comparison — all explicitly deferred.
3. **Large-N benchmarks** (n ≥ 10⁶) — not feasible in one session.
4. **Cross-platform / multi-machine verification** — needs CI matrix.
5. **Statistical CIs on the per-op costs** — most numbers are single-run.
