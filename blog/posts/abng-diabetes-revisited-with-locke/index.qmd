...
![Sunburst of all 142 Locke findings. Inner ring: 4 severity levels (error 1, warning 8, notice 75, info 58). Outer ring: the 15 unique E-codes within each severity. **`notice` dominates by area** — 75 findings, almost all of them E9073 (×44, duplicate-key disagreements — a single column's worth of disagreement counted ~40 times across 40 columns) and E9011 (×15, high-cardinality categoricals). The single `error` (E9004) is the patient_nbr duplicate-key finding called out in §4.1. **E9063 (multi-class leakage on `discharge_disposition_id`) is *not* in the sunburst** — it never fired, which is itself the post's load-bearing Locke finding.](figures/traditional/locke_findings_sunburst.png){#fig-findings-sunburst fig-alt="Sunburst chart of 142 Locke findings. Centre ring divided into 4 wedges by severity (error tiny, warning small, notice largest, info large). Outer ring shows individual E-codes within each severity wedge — E9002 dominates info, E9073 dominates notice. E9063 absent."}

![Pearson |r| heatmap — themed Plotly recreation of the actual SVG Locke emitted (raw verbatim version preserved at [`figures/locke/pearson_heatmap_raw.svg`](figures/locke/pearson_heatmap_raw.svg) for verification). The Pearson values are byte-identical to what `cjcl locke validate` produced. The only off-diagonal correlation that pops is `encounter_id` × `patient_nbr` (r = 0.512) — the patient-encounter overlap that E9004 separately flagged. Everything else is < 0.16 in absolute value, consistent with the rest of the dataset's heterogeneity.](figures/traditional/locke_pearson_heatmap.png){#fig-locke-heatmap fig-alt="Pearson r heatmap, 6x6 grid of numeric columns: encounter_id, patient_nbr, admission_type_id, discharge_disposition_id, num_lab_procedures, diag_1. Diagonal cells are 1.00 in bold pink. The encounter_id × patient_nbr cell shows r=0.512 in lighter pink, called out with an arrow. All other off-diagonals are near-zero in dark teal."}
...
![§4.B confounded-experiment flow. The prune does two things at once: **Effect A** removes the leakage rows (which would help on its own), but **Effect B** shifts the MI-selected routing-feature pair from `[num_inpatient, time_in_hospital]` to `[num_inpatient, number_emergency]` — a strictly weaker top-K=2 pair at 20K. The downstream MI shift dominated the leakage-removal gain. The footnote captures the deeper insight: the same MI pair `[num_inpatient, number_emergency]` wins at 101K scale anyway (§5.2), so Effect B was about to happen as data scaled. §4.B surfaced it early.](figures/traditional/confounded_experiment_flow.png){#fig-confounded-flow fig-alt="Flow diagram. Top yellow box: §4.B intervention (drop discharge codes 11,13,14,19,20,21, −2,423 rows). Two arrows branch down to: green Effect A box (Leakage rows removed) and orange Effect B box (MI re-selects routing features, time_in_hospital → number_emergency). Both arrows converge into a bottom orange Net Effect box (AUC drops 0.6312 → 0.6194, routing-feature loss > leakage-removal gain, marked as confounded experiment)."}
...
![§4.D Part 1's effect on leaf 1's 8-axis BeliefScore vector (n=9,617 rows, the largest leaf). The naive Locke polygon (grey, with default `ValidationConfig`) is mostly perfect — `missingness` at the top sits at the outermost ring (1.0 = "no missingness detected"). The `?`-aware polygon (blue) is *identical* on every axis except `missingness`, where it dents inward to 0.9621 — Locke now sees `?` strings as missing. That dent is the §4.D Part 1 fix made visible: targeted (one axis), not noisy (other axes unmoved). The deep concave at the right (`schema = 0.34`) is real Locke schema-concern data, present on both polygons.](figures/traditional/per_leaf_belief_radar.png){#fig-belief-radar fig-alt="Polar radar chart with 8 axes (schema, missingness, drift, leakage, lineage, sample, duplication, constraint). Two overlapping filled polygons: naive Locke (grey) and ?-aware (blue). The polygons overlap almost entirely except at the missingness axis, where the blue ?-aware polygon dents inward (0.9621) while the grey naive one stays at 1.0. The schema axis at the right shows a deep concave (0.34) on both polygons — Locke's schema_score is genuinely low on this dataset."}
...
![ABNG's per-leaf BLR ensemble structure. Each input row passes through the `CategoricalTransform` to become `(x, φ, y)` — `x` routes; `φ` predicts. The **root BLR** has its own conjugate posterior trained on all rows. The **routing decision** is frozen at graph-build time using the top-K=2 MI-scored columns. Each row then trains exactly *one* leaf's BLR, again with closed-form conjugate updates. At predict time, the ensemble is `0.5 × root_mean + 0.5 × routed_leaf_mean`. The schematic shows K=1 (4 leaves) for legibility — diabetes-130 actually uses K=2, fanning into 16 leaves. The audit chain SHA-256 hash advances per `train_step`, binding (row, leaf, posterior state) into one chained event.](figures/traditional/abng_architecture.png){#fig-abng-architecture fig-alt="Architecture flow diagram. Top: Row goes through CategoricalTransform to produce (x, φ, y). Two branches: one to Root BLR (purple, posterior on all data), one to Routing decision (orange, K=2 MI-selected features). Routing fans out to 4 Leaf BLR boxes (blue). All five — root and 4 leaves — converge into a bottom Ensemble box (green): Ensemble = 0.5 × root + 0.5 × routed leaf. Footnote: K=1 shown for legibility; actual K=2 means 16 leaves."}
